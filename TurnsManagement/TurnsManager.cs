...
public sealed class TurnsManager : ITurnsManager
{
	...
	public IReadOnlyList<int> ActivePlayers => activePlayers;

	public int Current
	{
		get => GetPlayer(currentPlayerIndex);
		set => currentPlayerIndex = activePlayers.IndexOf(value) switch
		{
	...
	public TurnsManager(int playersCount, TurnsDirection? direction = null)
	{
		...
		activePlayers = Enumerable.Range(0, playersCount).ToList();
	...
	public int Jump(int skippedTurns, TurnsDirection? direction = null)
	{
		...
		if (skippedTurns != 0)
		{
			...
			currentPlayerIndex = (currentPlayerIndex + skippedTurns) % activePlayers.Count;
			...
			Debug.Assert(currentPlayerIndex >= 0, "Current player is negative number!");
			Debug.Assert(currentPlayerIndex < activePlayers.Count, "Current is greater than players count!");
		}
	...
	public void RemovePlayer(int playerId)
	{
		...
		else
		{
			...
			if (currentPlayerIndex < 0)
			{
				currentPlayerIndex = activePlayers.Count == 0
					? 0
